...
class Automod(commands.Cog):
        ...
        async def enable_logging_callback(interaction):
            ...
            try:
                for channel in interaction.guild.channels:
                    if channel.name == "SB Moderation-automod":
                        await interaction.response.send_message(f"A logging channel with the name \"SB Moderation-automod\" already exists.", ephemeral=True)
                        return
                log_channel = await interaction.guild.create_text_channel("SB Moderation-automod", overwrites=overwrites)
    ...
    async def ignore_channel(self, ctx, channel: discord.TextChannel):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            success = discord.Embed(title="<:SB Moderation_tick:1227866641027698792> Channel Whitelisted", description=f"The channel {channel.mention} has been added to the ignore list \n\n➜ Use `{ctx.prefix}automod ignore show` to view the ignore list.", color=0x000000)
    ...
    async def ignore_role(self, ctx, role: discord.Role):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            success = discord.Embed(title="<:SB Moderation_tick:1227866641027698792> Role Whitelisted", description=f"The role {role.mention} has been added to the ignore list \n\n➜ Use `{ctx.prefix}automod ignore show` to view the ignore list.", color=0x000000)
            success.set_thumbnail(url=self.bot.user.avatar.url)
            success.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                   icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
    ...
    async def ignore_reset(self, ctx):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            await db.commit()
        embed=discord.Embed(title=f"Automod Settings for {ctx.guild.name}", description=f"** <:SB Moderation_tick:1227866641027698792> | All ignored channels and roles have been reset!**\n\nTo view current Automod settings use `{ctx.prefix}automod config`", color=0x000000)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
        embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
               icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        await ctx.send(embed=embed)
    ...
    async def unignore_channel(self, ctx, channel: discord.TextChannel):
        ...
        if result.rowcount > 0:
            embed = discord.Embed(title="<:SB Moderation_tick:1227866641027698792> Success", description=f"{channel.mention} has been removed from the automod ignore list.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title="<:SB Moderation_cross:1227866668152393789> Error", description=f"{channel.mention} is not in the automod ignore list.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
    ...
    async def unignore_role(self, ctx, role: discord.Role):
        ...
        if result.rowcount > 0:
            embed = discord.Embed(title="<:SB Moderation_tick:1227866641027698792> Success", description=f"{role.mention} has been removed from the automod ignore list.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title="<:SB Moderation_cross:1227866668152393789> Error", description=f"{role.mention} is not in the automod ignore list.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
    ...
    async def disable(self, ctx):
        ...
        if view.value is None:
            ...
            embed.title = "<:SB Moderation_tick:1227866641027698792> Automod Disabled"
    ...
    async def logging(self, ctx, channel: discord.TextChannel):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            await db.commit()
            embed=discord.Embed(title=f"Automod Settings for {ctx.guild.name}", description=f"**<:SB Moderation_tick:1227866641027698792> | Automoderation Logging channel set to {channel.mention}.**\n\n➜ Use `{ctx.prefix}automod config` to view current Automod settings.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                   icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        await ctx.send(embed=embed)
