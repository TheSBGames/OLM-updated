...
class Antinuke(commands.Cog):
  ...
  async def antinuke(self, ctx, option: str = None):
    ...
    if not is_owner and not check:
      embed = discord.Embed(title="<:SB Moderation_cross:1227866668152393789> Access Denied",
                color=0x000000,
    ...
    if option is None:
      ...
      if is_activated:
        ...
        if not ctx.guild.me.guild_permissions.administrator:
          setup_embed.description += "\n<a:SB Moderation_WarnFlash:1272569018183843874> | Setup failed: Missing **Administrator** permission."
          await setup_message.edit(embed=setup_embed)
          return

        await asyncio.sleep(1)
        setup_embed.description += "\n<:setup_success:1261574239354621963> | Checking SB Moderation's role position for optimal configuration..."
        await setup_message.edit(embed=setup_embed)

        await asyncio.sleep(1)
        setup_embed.description += "\n<:setup_success:1261574239354621963> | Crafting and configuring the SB Moderation Supreme role..."
        await setup_message.edit(embed=setup_embed)
        
        try:
          role = await ctx.guild.create_role(
            name="SB Moderation Supreme™",
            color=0xdc143c,
          ...
          setup_embed.description += "\n<a:SB Moderation_WarnFlash:1272569018183843874> | Setup failed: Insufficient permissions to create role."
          await setup_message.edit(embed=setup_embed)
          return
        except discord.HTTPException as e:
          setup_embed.description += f"\n<a:SB Moderation_WarnFlash:1272569018183843874> | Setup failed: HTTPException: {e}\nCheck Guild **Audit Logs**."
          await setup_message.edit(embed=setup_embed)
          return

        await asyncio.sleep(1)
        setup_embed.description += "\n<:setup_success:1261574239354621963> | Ensuring precise placement of the SB Moderation Supreme role..."
        await setup_message.edit(embed=setup_embed)
        try:
          await ctx.guild.edit_role_positions(positions={role: 1})
        except discord.Forbidden:
          setup_embed.description += "\n<a:SB Moderation_WarnFlash:1272569018183843874> | Setup failed: Insufficient permissions to move role."
          await setup_message.edit(embed=setup_embed)
          return
        except discord.HTTPException as e:
          setup_embed.description += f"\n<a:SB Moderation_WarnFlash:1272569018183843874> | Setup failed: HTTPException: {e}."
