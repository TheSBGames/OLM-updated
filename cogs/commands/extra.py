...
tick = "<:SB Moderation_tick:1227866641027698792>"
cross = "<:SB Moderation_cross:1227866668152393789>"
...
class Extra(commands.Cog):
  ...
  async def uptime(self, ctx):
      ...
      embed = discord.Embed(title=f"SB Moderation Uptime", color=self.color)
      embed.add_field(name="__UTC__", value=f"<:SB Moderation_info:1227625594511163537> {uptime_string}\n\n", inline=False)
      embed.add_field(name="__Online Duration__", value=f"<:SB Moderation_duration:1251052632949395538> {uptime_duration_string}", inline=False)
  ...
  async def serverinfo(self, ctx):
        ...
        embed.add_field(
            name="**__About__**",
            value=f"**Name : ** {ctx.guild.name}\n**ID :** {ctx.guild.id}\n**Owner <:SB Moderation_owner:1228227536207740989> :** {ctx.guild.owner} (<@{ctx.guild.owner_id}>)\n**Created At : ** {c_at}\n**Members :** {len(ctx.guild.members)}",
            inline=False
        )
        ...
        embed.add_field(
            ...
            value=f"Level: {ctx.guild.premium_tier} [<:SB Moderation_booster:1243448375328903199> {ctx.guild.premium_subscription_count} boosts]",
            inline=False
        )
...
**ID:** {member.id}
**Nickname:** {nickk}
**Bot?:** {'<:SB Moderation_tick:1227866641027698792> Yes' if member.bot else '<:SB Moderation_cross:1227866668152393789> No'}
**Badges:** {badges}
...
**Color:** {member.color if member.color else '99aab5'}
  ...
  async def boosts(self, ctx):
    await ctx.send(
      embed=discord.Embed(title=f"<:SB Moderation_booster:1243448375328903199> Boosts Count Of {ctx.guild.name}",
