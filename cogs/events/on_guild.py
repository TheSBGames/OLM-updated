...
from core import SB Moderation, Cog
import discord
import logging
...
class Guild(Cog):
    def __init__(self, client: SB Moderation):
        self.client = client
    ...
    async def on_guild_add(self, guild):
        try:
            ...
            embed.add_field(
                name="**__Members__**",
                value=f"""<:SB Moderation_users:1226064812157571163> Members : {len(guild.members)}\n<:SB Moderation_friend:1228229690376982549> Humans : {len(list(filter(lambda m: not m.bot, guild.members)))}\n<:SB Moderation_bot:1227873668466741320> Bots : {len(list(filter(lambda m: m.bot, guild.members)))}
                """,
                inline=False
            )
            embed.add_field(
