...
class TimeoutView(ui.View):
    ...
    @ui.button(style=discord.ButtonStyle.gray, emoji="<:SB Moderation_bin:1254336650075701308>")
...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(description=f"**<:SB ModerationUser:1294654665895579721> Target User:** [{self.user}](https://discord.com/users/{self.user.id})\n<:SB ModerationMention:1294654604998475856> **User Mention:** {self.user.mention}\n**<:SB ModerationDM:1295595078122999915> DM Sent:** {dm_status}\n**<:SB ModerationReason:1295595129809141812> Reason:** {reason}", color=0x000000)
        ...
        embed.add_field(name="<:SB ModerationMod:1295601558985379852> Moderator:", value=interaction.user.mention, inline=False)
...
class Mute(commands.Cog):
    ...
    async def mute(self, ctx, user: discord.Member, time: str = None, *, reason=None):
        ...
        try:
            await user.send(f"<a:SB Moderation_WarnFlash:1272569018183843874> You have been muted in **{ctx.guild.name}** by **{ctx.author}** for {duration_text}. Reason: {reason or 'None'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"**<:SB ModerationUser:1294654665895579721> Target User:** [{user}](https://discord.com/users/{user.id})\n"
                                          f"<:SB ModerationMention:1294654604998475856> **User Mention:** {user.mention}\n"
                                          f"<:SB ModerationDM:1295595078122999915> **DM Sent:** {dm_status}\n"
                                          f"**<:SB ModerationReason:1295595129809141812> Reason:** {reason or 'None'}\n"
                                          f"**<:SB ModerationTime:1294654567539277824> Duration:** {duration_text}",
                              color=self.color)
        embed.set_author(name=f"Successfully Muted {user.name}", icon_url=self.get_user_avatar(user))
        embed.add_field(name="<:SB ModerationMod:1295601558985379852> Moderator:", value=ctx.author.mention, inline=False)
        embed.set_footer(text=f"Requested by {ctx.author}", icon_url=self.get_user_avatar(ctx.author))
        embed.timestamp = discord.utils.utcnow()
